...
%tempH=InputWeight*P;
tempH=Matrix_Muplication_mex(InputWeight,P);
...
%OutputWeight=Matrix_Inverse_mex(H,T);
...
OutputWeight_temp=(eye(size(H,1))/C + Matrix_Muplication_mex(H,H'))\ H;
OutputWeight=Matrix_Muplication_mex(OutputWeight_temp,T');
%implementation; one can set regularizaiton factor C properly in classification applications
...
Y=Matrix_Muplication_mex(H',OutputWeight);
Y=Y';
...
tempH_test=Matrix_Muplication_mex(InputWeight,TV.P);
...
TY=Matrix_Muplication(H_test',OutputWeight); 
TY=TY';
